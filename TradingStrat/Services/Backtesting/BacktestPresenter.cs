...
{
    public void DisplayResults(BacktestResult result)
    {
    ...
    private void DisplayHeader(BacktestResult result)
    {
        var panel = new Panel(
    ...
    private void DisplayPerformanceSummary(PerformanceMetrics metrics)
    {
    ...
    private void DisplayTradeHistory(List<Trade> trades)
    {
        if (trades.Count == 0)
        {
    ...
    private void DisplayEquityCurveSummary(List<EquityPoint> equityCurve)
    {
    ...
    private string FormatCurrency(decimal value, bool isNegative = false)
    {
        var color = isNegative ? "red" : "cyan";
        return $"[{color}]${Math.Abs(value):N2}[/]";
    }

    private string FormatProfitLoss(decimal value)
    {
        if (value > 0)
    ...
    private string FormatPercentage(decimal value, bool isNegative = false)
    {
