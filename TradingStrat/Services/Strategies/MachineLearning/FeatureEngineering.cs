...
public class FeatureEngineering
{
        ...
        BaseStrategy strategyBase)
    {
        ...
        CalculateAllIndicators(strategyBase);
    }

    private void CalculateAllIndicators(BaseStrategy strategy)
    {
        // Use BaseStrategy methods where available
        _sma5 = strategy.CalculateSMA(5);
        _sma10 = strategy.CalculateSMA(10);
        _sma20 = strategy.CalculateSMA(20);
        _ema12 = strategy.CalculateEMA(12);
        _ema26 = strategy.CalculateEMA(26);
        _rsi14 = strategy.CalculateRSI(14);
        (_macd, _macdSignal, _macdHistogram) = strategy.CalculateMACD(12, 26, 9);

        // Calculate custom indicators
        _stdDev10 = CalculateRollingStdDev(10);
        _stdDev20 = CalculateRollingStdDev(20);
        _atr14 = CalculateATR(14);
    ...
    private decimal[] CalculateRollingStdDev(int period)
    {
    ...
    {
        var atr = new decimal[_closePrices.Length];
        var trueRanges = new decimal[_closePrices.Length];

        for (int i = 0; i < _closePrices.Length; i++)
        {
            if (i == 0)
            {
                trueRanges[i] = _highPrices[i] - _lowPrices[i];
            }
            else
            {
                var highLow = _highPrices[i] - _lowPrices[i];
                var highClose = Math.Abs(_highPrices[i] - _closePrices[i - 1]);
                var lowClose = Math.Abs(_lowPrices[i] - _closePrices[i - 1]);
        ...
        for (int i = 0; i < atr.Length; i++)
        {
            ...
            {
                atr[i] = trueRanges.Skip(0).Take(period).Average();
            }
            else
            {
