...
/// <summary>
/// Base class for use cases that eliminates try-catch boilerplate.
...
public abstract class BaseUseCase<TCommand, TResult>
{
    /// <summary>
    ...
    protected async Task<Result<TResult>> ExecuteAsync(
        TCommand command,
        Func<TCommand, Task<TResult>> executeCore,
        string errorCode)
    {
        try
        {
            TResult result = await executeCore(command);
