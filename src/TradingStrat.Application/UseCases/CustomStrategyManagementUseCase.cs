...
/// NOW A FACADE: Delegates to specialized use cases for queries, commands, and Python validation.
/// Maintained for backward compatibility with existing Web layer code.
/// Prefer using specialized use cases directly.
/// </summary>
[Obsolete("Split into specialized use cases for better separation of concerns. " +
          "Use ICustomStrategyQueryUseCase for reads, ICustomStrategyCommandUseCase for writes, " +
          "ValidatePythonCodeUseCase and DryRunPythonStrategyUseCase for Python validation.")]
public class CustomStrategyManagementUseCase : ICustomStrategyManagementUseCase
{
    ...
    private readonly ValidatePythonCodeUseCase _validatePythonCodeUseCase;
    private readonly DryRunPythonStrategyUseCase _dryRunPythonStrategyUseCase;
    ...
    public CustomStrategyManagementUseCase(
        ...
        ValidatePythonCodeUseCase validatePythonCodeUseCase,
        DryRunPythonStrategyUseCase dryRunPythonStrategyUseCase)
    {
        _queryUseCase = queryUseCase;
        _commandUseCase = commandUseCase;
        _validator = validator;
        _validatePythonCodeUseCase = validatePythonCodeUseCase;
        _dryRunPythonStrategyUseCase = dryRunPythonStrategyUseCase;
    }
    ...
    // Validation operations - delegate to domain service and Python use cases
    ...
    public Task<Result<AppPythonValidationResult>> ValidatePythonCodeAsync(ValidatePythonCodeCommand command)
        => _validatePythonCodeUseCase.ExecuteAsync(command);

    public Task<Result<DryRunResult>> DryRunPythonStrategyAsync(DryRunPythonStrategyCommand command)
        => _dryRunPythonStrategyUseCase.ExecuteAsync(command);
}
